...
function loadEvents() {
  ...
  if (saved && saved !== '[]') {
    try {
      ...
      if (!Array.isArray(events) || events.length === 0) events = [...DEFAULT_EVENTS];
    } catch (e) {
...
function loadLocations() {
  ...
  if (saved && saved !== '[]') {
    try {
      ...
      if (!Array.isArray(locations) || locations.length === 0) locations = [...DEFAULT_LOCATIONS];
    } catch (e) {
...
// Safari SVG filter compatibility MutationObserver
const highlightObserver = new MutationObserver((mutationsList) => {
  for (const mutation of mutationsList) {
    if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
      ...
      if (isHighlighted) {
        target.setAttribute('filter', 'url(#line-glow-filter)');
      } else {
...
function observeAllConnectorPaths() {
  document.querySelectorAll('.connector-line, .spouse-connector, .preacher-line, .custom-visual-line').forEach(el => {
    if (el.classList.contains('line-highlight')) {
      el.setAttribute('filter', 'url(#line-glow-filter)');
    } else {
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    if (exactMatches.length > 1 && searchResults) {
      ...
      exactMatches.forEach(matched => {
        ...
        li.innerHTML = `<strong>${matched.name}</strong> ${parentInfo}`;
...
function saveAdminForm() {
  ...
  for (let tId of teachers) {
    if (!db.some(c => c.id === tId)) {
      alert(`전도사 ID '${tId}'가 데이터베이스에 존재하지 않습니다.`);
      return;
    }
  }
...
function openLayerDetails(data, type) {
  ...
  if (titleEl) titleEl.innerHTML = `<span style="font-size: 0.8em; color: #888;">${type === 'event' ? '📜 사건' : '📍 장소'}</span><br>${data.name}`;
...
function setupLayerItemModalEvents() {
  ...
  if (layerItemForm) {
    layerItemForm.addEventListener('submit', (e) => {
      ...
      const checkedPeople = Array.from(document.querySelectorAll('#layer-item-people-list input[type="checkbox"]:checked'))
        .map(cb => cb.value);
      const checkedEvents = Array.from(document.querySelectorAll('#layer-item-events-list input[type="checkbox"]:checked'))
        .map(cb => cb.value);
      const checkedLocations = Array.from(document.querySelectorAll('#layer-item-locations-list input[type="checkbox"]:checked'))
        .map(cb => cb.value);
...
(function() {
  ...
  document.addEventListener('contextmenu', (e) => {
    // Allow right click ONLY if they are clicking inside input or textarea elements (for normal editing)
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (isMetaOrCtrl && isShift && (e.key === 'i' || e.key === 'I')) {
      e.preventDefault();
      return false;
    }
    if (isMetaOrCtrl && isAlt && (e.key === 'i' || e.key === 'I')) {
      e.preventDefault();
      return false;
    }
    
    // Console: Ctrl+Shift+J / Cmd+Option+J
    if (isMetaOrCtrl && isShift && (e.key === 'j' || e.key === 'J')) {
      e.preventDefault();
      return false;
    }
    if (isMetaOrCtrl && isAlt && (e.key === 'j' || e.key === 'J')) {
      e.preventDefault();
      return false;
    }
  ...
  }, true); // Use capture phase to intercept shortcuts early
  
  // 3. DevTools Debugger Loop (Freezes page execution if DevTools is open)
  ...
  // 4. Secret developer unlock code (Type "unlockdev" on keyboard to disable protection for developer debugging)
  let inputBuffer = "";
  window.addEventListener('keydown', (e) => {
